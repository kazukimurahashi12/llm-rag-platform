...
ONBOARD-Core は、ONBOARD AI プロダクトへの組み込みを想定して設計した、マネジメント支援 AI バックエンドの MVP 兼ポートフォリオです。単なるチャット UI ではなく、高精度（RAG）、高信頼（ガードレール）、高透明性（コスト・監査） を備えた、実運用可能な API 基盤を目指しています。
...
<img width="1436" height="657" alt="スクリーンショット 2026-04-22 13 03 16" src="https://github.com/user-attachments/assets/deb733fb-8c37-4912-a328-3afabe88e566" />
...
### Week 1: LLM Gateway + 監査・メーター

- `/v1/chat`
- token / cost 計測
- `audit_log` を PostgreSQL に保存
- Redis による rate limit

### Week 2: RAG（pgvector）+ ドキュメント投入

- `documents` テーブルと embedding 保存
- chunking
- retrieval TopK
- 回答時の根拠引用
...
- metrics 可視化
- README で設計判断を説明
- `docker compose` によるデモ手順整備

## 今後追加する主要機能

- RAG 用知識投入 API
- 根拠文書つき回答 API
- PII / safety check API
- audit logging 永続化
- モデル別料金マスタ
- latency / token / error rate のメトリクス公開
- 評価用データセットと自動採点基盤
...
- チャンク粒度をどう決めるか
- rerank をどこで入れるべきか
- groundedness / cost / latency をどう両立するか
- PII をどこで mask し、何をログに残すか
- prompt injection に対して何が防げて何が防げないか
- 1 リクエストあたりの unit economics をどう見るか
