...
export async function fetchKnowledgeDocuments(params: Record<string, string | number | undefined>) {
  const response = await apiClient.get<KnowledgeDocumentListResponse>("/v1/knowledge-documents", {
    params,
  });
...
export async function updateKnowledgeDocument(
  ...
  const response = await apiClient.put<KnowledgeDocumentResponse>(
    `/v1/knowledge-documents/${knowledgeDocumentId}`,
    payload,
  );
  return response.data;
}
...
export async function reindexAllDocuments() {
  const response = await apiClient.post<KnowledgeReindexJobAcceptedResponse>(
    "/v1/knowledge-documents/reindex",
  );
  return response.data;
}
