...
export function ReindexJobsPage() {
  ...
  return (
    ...
    >
      <SectionCard
        title="ジョブ絞り込み"
        description="状態で絞り込むと、失敗中や実行中のジョブを追いやすくなります。"
      >
        <TextField
          select
          label="状態"
      ...
      <SectionCard
        title="ジョブ履歴"
        description="5秒ごとのポーリングで、実行中ジョブを手動更新なしで追跡します。"
      >
        {!authSession ? (
          <EmptyState
            title="サインインが必要です"
        ...
        {retryMutation.isSuccess ? (
          <Alert severity="success">再試行ジョブを受け付けました。</Alert>
        ) : null}
        ...
        {retryMutation.isError ? (
          <ErrorState message={getApiErrorMessage(retryMutation.error)} />
        ) : null}
        ...
        {listQuery.data && listQuery.data.items.length === 0 ? (
          <EmptyState
            title="再インデックスジョブはありません"
            body="ナレッジ画面から再インデックスを実行すると、ここに履歴が表示されます。"
          />
        ) : null}
        {listQuery.data ? (
          <Table>
